...
from pdftabextract.clustering import calc_cluster_centers_1d, zip_clusters_and_values, find_clusters_1d_break_dist
        ...
        for table_index, table in enumerate(self.tables):
            ...
            try:
                ...
                for hor in page['hor_lines']:
                    ...
                    if rectintersect(table_bbox, hor_bbox) > .3 and width_i > .25 * img_orig.shape[1]:
                ...
                for vert in page["vert_lines"]:
                    ...
                    if rectintersect(table_bbox, vert_bbox) > .5 and height_i > .15 * table_height:
                        initial_cols.append((top_i, bottom_i, (left_i + right_i) / 2))

                if initial_cols and initial_rows:
                    table["modified_top"] = modified_top = int(np.median(list(map(lambda item: item[0], initial_cols))))
                    table["modified_bottom"] = modified_bottom = int(np.median(list(map(lambda item: item[1], initial_cols))))
                    table["modified_left"] = modified_left = int(np.median(list(map(lambda item: item[0], initial_rows))))
                    table["modified_right"] = modified_right = int(np.median(list(map(lambda item: item[1], initial_rows))))
...
def transform_fn(none_model, data, input_content_type, output_content_type):
    ...
    # loc_data = {'bucket': bucket, 'file_name': image_file_name_s3}
