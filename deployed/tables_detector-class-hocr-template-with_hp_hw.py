...
subprocess.call([sys.executable, '-m', 'pip', 'install', 'sagemaker==1.13.0'])  # ==1.14.0
...
# from pdftabextract.geom import rectintersect
...
class HocrDocument(object):
    ...
    def get_pages_data(self):
        ...
        for idx, page_node in enumerate(page_node_iter):
            ...
            pages_data.append((page_id, lines_per_page, words_per_page, hor_per_page, vert_per_page))
        return pages_data
    ...
    def parse(self):
        if self.parse_data is None:
            ...
            for pid, lines_s, words_s, hor_s, vert_s in self.get_pages_data():
                texts[pid] = [sentence for _, _, word_x1_x2, sentences in lines_s for sentence in sentences]
                words[pid] = words_s
    ...
    def add_from_deployed(self, page_id, json_input, predictor):
        ...
        for i, (x1_loc_i, y1_loc_i, w_loc_i, h_loc_i) in enumerate(localization_bboxes):
            ...
            for words_index, word in enumerate(self.parse_data[page_id]["words"]):
                ...
                inter = rectintersect(loc_bbox, tesseract_bbox)  # tesseract_bbox inside loc_bbox
                if inter is not None and inter > .8 and word["height"] > 50 and word["width"] > 50:  # mostly, localization boxes are larger than needed
...
class TableDetector:
    ...
    def visualize_tables(self, img_orig):
        tables = self.tables
        tables_img = img_orig
        line_width = 10
        # fig = plt.figure()
    ...
    def fit_text_to_tables(self):
        ...
        for table in self.tables:
            ...
            median_text_height_ps = np.median([t_ps['height'] for t_ps in page["sentences"]])
            ...
            texts_cols_ps = [t_ps for t_ps in page["sentences"]
            ...
            try:
                ...
                for word in page["words"]:
                    ...
                    if intersection_index(left_i, right_i, table_left, table_right) > .7 and intersection_index(top_i, bottom_i, table_top, table_bottom) > .7:  # and len(word["value"].strip()) == 0
                        words.append(word)
                ...
                for hor in page['hor_lines']:
                    ...
                    if intersection_index(left_i, right_i, table_left, table_right) > .5 and intersection_index(top_i, bottom_i, table_top, table_bottom) > .5 and width_i >= .5 * table_width:  # row
                ...
                for vert in page["vert_lines"]:
                    ...
                    if intersection_index(left_i, right_i, table_left, table_right) > .5 and intersection_index(top_i, bottom_i, table_top, table_bottom) > .5 and height_i >= .5 * table_height:  # column
                        cols.append((top_i, bottom_i, (left_i + right_i) / 2))

                if cols and rows:
                    table["modified_top"] = modified_top = int(np.median(list(map(lambda item: item[0], cols))))
                    table["modified_bottom"] = modified_bottom = int(np.median(list(map(lambda item: item[1], cols))))
                    table["modified_left"] = modified_left = int(np.median(list(map(lambda item: item[0], rows))))
                    table["modified_right"] = modified_right = int(np.median(list(map(lambda item: item[1], rows))))
        ...
        for i, table_df in enumerate(self.tables_df):
            ...
            tables_structure.append({"table": i,
                                     ...
                                     "coordinates": [{'x1': int(self.tables[i]['col_positions'][0]), 'y1': int(self.tables[i]['table_start']), 'x2': int(self.tables[i]['col_positions'][-1]), 'y2': int(self.tables[i]['table_end'])}]
                                        , "modified coordinates": [{'x1': self.tables[i]["modified_left"], 'y1': self.tables[i]["modified_top"], 'x2': self.tables[i]["modified_right"],
                                                                    'y2': self.tables[i]["modified_bottom"]}]
...
def intersection_index(first_min, first_max, second_min, second_max):
    intersect_min = max(first_min, second_min)
    intersect_max = min(first_max, second_max)

    return max(0, (intersect_max - intersect_min) / min(second_max - second_min, first_max - first_min))
...
def transform_fn(none_model, data, input_content_type, output_content_type):
    global page_scaling_vert, tess_scaling_vert, img_orig, page_scaling_hor, tess_scaling_hor, page, s3, img_for_predictors
    ...
    loc_data = {'bucket': bucket, 'file_name': image_file_name}
    print("sending to localizer:" + os.path.join(remote_dir_for_output, small_image_name))
    loc_data = {'bucket': bucket, 'file_name': os.path.join(remote_dir_for_output, small_image_name)}
    loc_out = loc_predictor.predict(loc_data)

    hw_data = {"bucket": loc_out["bucket_name"], "file_name": loc_out["hw_key"]}
    hp_data = {"bucket": loc_out["bucket_name"], "file_name": loc_out["hp_key"]}
    ...
    repository.add_from_deployed(list(pages.keys())[0], hw_data, hw_predictor)

    print("Calling Handprinting OCR...")
    repository.add_from_deployed(list(pages.keys())[0], hp_data, hp_predictor)
    #########################################################
    # import threading
    # print("Calling Handwriting OCR...")
