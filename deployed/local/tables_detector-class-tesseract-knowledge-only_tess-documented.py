...
def transform_fn(none_model, data, input_content_type, output_content_type):
    ...
    demo_samples.append(("image1.hocr", "pre_image1.tiff"))
    demo_samples.append(("image5.hocr", "pre_image5.tiff"))
    demo_samples.append(("pre_image1-imageinput-sandwich.hocr", "pre_image1-magick.tiff"))  # missing column
    demo_samples.append(("pre_image2-imageinput-sandwich.hocr", "pre_image2-magick.tiff"))  # missing column
    demo_samples.append(("pre_image3-imageinput-sandwich.hocr", "pre_image3-magick.tiff"))  # missing column
    demo_samples.append(("pre_image4-imageinput-sandwich.hocr", "pre_image4-magick.tiff"))
    demo_samples.append(("pre_image5-imageinput-sandwich.hocr", "pre_image5-magick.tiff"))  # missing column
    demo_samples.append(("pre_image5--imageinput-sandwich.hocr", "pre_image5--magick.tiff"))
    # # ======================================================================================
    # # some handwriting
    demo_samples.append(("Accident-Handwritten_V3-sandwich-2t.hocr", "Accident-Handwritten_V3_2.tiff"))
    demo_samples.append(("96be1cf1-b5e9-40c6-bfa8-58164e341e09$4.hocr", "96be1cf1-b5e9-40c6-bfa8-58164e341e09$4.tiff"))  # missing column
    demo_samples.append(("Accident Claim - 5-pdfinput-sandwich-3.hocr", "Accident Claim - 5_3.tiff"))
    demo_samples.append(("Accident Claim - 6-pdfinput-sandwich-3.hocr", "Accident Claim - 6_3.tiff"))
    demo_samples.append(("Image.hocr", "Accident Claim - 8.tiff"))
    demo_samples.append(("Accident Claim - 8-pdfinput-sandwich-3.hocr", "Accident Claim - 8_3.tiff"))  # ........
    demo_samples.append(("Accident Claim - 7-pdfinput-sandwich-3.hocr", "Accident Claim - 7_3.tiff"))  # very bad document
    # # ======================================================================================
    # # all handwriting
    demo_samples.append(("Accident Claim - 2-pdfinput-sandwich-3.hocr", "Accident Claim - 2_3.tiff"))
    # # ======================================================================================
    # # all printed samples
    demo_samples.append(("page1.hocr", "Page 1.tiff"))  # ....
    # # ======================================================================================
    # # noisy :) samples
    demo_samples.append(("noisy example-imageinput-sandwich.hocr", "noisy example-magick.tiff"))
    demo_samples.append(("noisy example--imageinput-sandwich.hocr", "noisy example--magick.tiff"))  # [] ....
    # # ======================================================================================
    # borderless samples
    demo_samples.append(("Confirmation of  Coverage - 1-pdfinput-sandwich.hocr", "Confirmation of  Coverage - 1_0.tiff"))

    for i in range(0, 14):
        demo_samples.append(("250091970-pdfinput-sandwich-{}.hocr".format(i), "250091970_{}.tiff".format(i)))

    # # ======================================================================================
    #
    # communicating with endpoints

    for i, (hocr_file_name, image_file_name) in enumerate(demo_samples):
        # as if I'm reading the json
        print("Getting Your files...")
        hocr_file_name = "hocr-files/" + hocr_file_name
        image_file_name = "image-files/" + image_file_name
        print(i, hocr_file_name, image_file_name)

        # as if loading from s3
        s3 = boto3.resource('s3', aws_access_key_id='', aws_secret_access_key='')
        session = boto3.Session(region_name='us-west-2', aws_access_key_id='', aws_secret_access_key='')

        # as if creating predictors
        ...
        cv2.imwrite("small-image-files/"+small_image_name, img_for_predictors)

        # as if uploading smaller image
        # upload_file(s3, bucket, open(small_image_name, "rb"), os.path.join(remote_dir_for_output, small_image_name))

        # modifying the image for table detector
        img = (255 - copy.copy(img_orig)) / 255  # for histogram projection
        img = img[:, :-100].astype(np.float32)

        print("Parsing the document..")
        repository = HocrDocument(hocr_file_name)  # the hocr parser
        # repository.write_equivalent_xml()

        # making the structured format
        pages = repository.parse()
        pages_h = repository.hierarchical_parse

        # we have a single page document
        ...
        ################################################################### GRAPHICAL DEBUGGING ##############################################################
        # here I generate images for debugging, showing the tesseract annotations with colors

        debug_image_root_path = "debug-images"  # root path to save images
        iden = 0  # identification for image names
        ...
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_words{}.jpg".format(iden)), img_orig_words)
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_rand{}.jpg".format(iden)), img_orig_rand)
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_lines{}.jpg".format(iden)), img_orig_sentences)
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_hp_hw{}.jpg".format(iden)), img_orig_hp_hw)
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_words_sent{}.jpg".format(iden)), img_orig_words_sent)
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_words_lines{}.jpg".format(iden)), img_orig_words_lines)
        cv2.imwrite(os.path.join(debug_image_root_path, "img_orig_hier{}.jpg".format(iden)), img_orig_hier)
        ####################################################################################################################################################
        ...
        tables_detector = TableDetector(verbose=False, strip_height=50, w_max_pool=75, min_col_width=250, ratio_clip_max=0.25)
        print("Detecting bordered Tables..")
        tables_detector.detect_tables(img)

        print("Fitting bordered Tables Text..")
        tables_detector.fit_bordered_tables()

        print("Detecting borderless Tables..")
